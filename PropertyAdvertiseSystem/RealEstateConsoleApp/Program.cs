...
//1 - read db records - connection command - connection string - get api
//2 - for each reccord
// 3 - type 

			// ساخت درخت به صورت خودکار
			MenuItem root = CreateMenuTree();

			// تبدیل درخت به جیسون
			string jsonOutput = JsonConvert.SerializeObject(root, Formatting.Indented);

			// ذخیره کردن خروجی در فایل جیسون
			string filePath = "menu_tree.json"; // مسیر فایل خروجی
			...
			static MenuItem CreateMenuTree()
			{
				// ساخت ریشه درخت
